    ...
    fun repaintCanvas() {
        ...
        if (surface != null) {
            ...
            if (streamSource == StreamSource.CANVAS_STREAM_ACQUIRED) {
                ...
                val eglImageOES = NativeHelper.hwBufferToEglImage(image.hardwareBuffer!!)
                val releaseImage = Runnable {
                    image.close()
                    NativeHelper.destroyEglImage(eglImageOES)
                }
                filamentStream!!.setAcquiredImage(eglImageOES, directImageHandler, releaseImage)
            }
        }
